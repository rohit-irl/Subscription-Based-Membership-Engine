...
const DEFAULT_DOWNLOAD_URL =
  process.env.DOWNLOAD_URL || `http://localhost:${PORT}/downloads/sample.pdf`;
...
// Generate a minimal valid PDF on the fly (so /download always works).
function generateSamplePdfBytes(text) {
  const safeText = escapePdfString(text || 'CapySphere Sample PDF');

  const header = '%PDF-1.4\n%Generated by CapySphere\n';

  const streamContent = `BT /F1 24 Tf 20 100 Td (${safeText}) Tj ET`;
  ...
  for (let i = 0; i < objects.length; i++) {
    offsets[i + 1] = currentOffset; // object numbers start at 1
...
app.get('/downloads/sample.pdf', (req, res) => {
  try {
